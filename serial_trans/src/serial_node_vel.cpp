...
void ArmCallback(const control_msgs::FollowJointTrajectoryActionGoal& msg) {
  ...
  /*
    position_msg.data.at(0) = msg.goal.trajectory.points[i].positions[0] * 30 *
    180 / PI; position_msg.data.at(1) =
    msg.goal.trajectory.points[i].positions[1] * 205 * 180 / (3 * PI);
    position_msg.data.at(2) = msg.goal.trajectory.points[i].positions[2] * 50 *
    180 / PI; position_msg.data.at(3) =
    msg.goal.trajectory.points[i].positions[3] * 125 * 180 / (2 * PI);
    position_msg.data.at(4) = msg.goal.trajectory.points[i].positions[4] * 125 *
    180 / (2 * PI); position_msg.data.at(5) =
